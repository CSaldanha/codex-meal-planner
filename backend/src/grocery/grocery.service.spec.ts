...
describe('GroceryService', () => {
    ...
    days: [
      ...
      },
      {
        ...
        meals: {
          breakfast: { recipeId: 1, recipeName: 'Scrambled Eggs', mealType: 'breakfast' },
          lunch: { recipeId: 2, recipeName: 'Pancakes', mealType: 'breakfast' },
          dinner: { recipeId: 2, recipeName: 'Pancakes', mealType: 'breakfast' }
        }
      }
    ],
    createdAt: new Date(),
  ...
  describe('getCurrentList', () => {
    it('should return grocery list for current meal plan', async () => {
      ...
      expect(mockMealPlansService.summarizePlan).toHaveBeenCalledWith(mockMealPlan);
  ...
  describe('getListForPlan', () => {
    it('should return grocery list for specific plan', async () => {
      mockMealPlansService.getPlanSummaryById.mockResolvedValue(mockMealPlanSummary);
      mockIngredientsService.findCheaperAlternative.mockResolvedValue(null);
  ...
  describe('createFromSummary', () => {
    it('should aggregate ingredients correctly with usage multipliers', async () => {
      ...
      const eggsItem = result.items.find(item => item.ingredient.name === 'Eggs');
      const milkItem = result.items.find(item => item.ingredient.name === 'Milk');
    ...
    it('should calculate potential savings with substitutions', async () => {
      ...
      mockIngredientsService.findCheaperAlternative
        .mockImplementation((ingredient: Ingredient) => {
          if (ingredient.id === 1) {
